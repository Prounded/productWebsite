...
if (localStorage.getItem('currentDisplay') === 'receipt') {
    ...
    function load() {
		...
		paymentMethodDisplay.addEventListener('click', changePaymentMethod);

    }

	function changePaymentMethod() {
		const currentPaymentMethod = localStorage.getItem('paymentMethod');
		const currentIndex = PAYMENT_METHODS.indexOf(currentPaymentMethod);
		const nextIndex = (currentIndex + 1) % PAYMENT_METHODS.length;
		localStorage.setItem('paymentMethod', PAYMENT_METHODS[nextIndex]);
		document.querySelector('.paymentMethodDisplay').textContent = PAYMENT_METHODS[nextIndex];
	}
					...
					if (currentItem.amount > 0) {
						...
						for (let j = 1; j <= Object.keys(purchaseHistory).length; j++) {
							if (
								...
								purchaseHistory[j].payment === localStorage.getItem('paymentMethod')
							) {
